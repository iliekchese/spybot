		...
		switch (interaction.options.getSubcommand()) {
			...
			case 'remove':
				if (interaction.user.id === interaction.guild.ownerId) {
					...
					const whitelist = db.get(`whitelist_${interaction.guild.id}`);
					if (whitelist) {
						const whitelistedUser = whitelist.find((x) => x.user === user.id);
						if (whitelistedUser) {
						...
						whitelist.pop(index);
						db.set(`whitelist_${interaction.guild.id}`, whitelist);
			...
			case 'show':
				...
				const whitelisted = db.get(`whitelist_${interaction.guild.id}`);
				if (whitelisted && whitelisted.length) {
					const users = whitelisted.map((x) => `<@${x.user}>`);
