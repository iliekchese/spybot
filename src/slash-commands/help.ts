...
export default {
	data: new SlashCommandBuilder().setName('help').setDescription('Displays help command!'),
	async run({ interaction }) {
		...
		const row = new MessageActionRow().addComponents(
			new MessageButton()
				...
				.setLabel('Invite')
				.setStyle('LINK'),
			new MessageButton().setURL('https://eldiplayz.gitbook.io/spy-bot-docs/').setLabel('Docs').setStyle('LINK'),
			new MessageButton().setURL('https://discord.gg/5qv5sHBPew').setLabel('Support Server').setStyle('LINK')
		);
